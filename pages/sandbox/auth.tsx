...
export default function AuthSandbox() {
  ...
  const { data, error } = useSWR('/api/sandbox/authDemo', async (url) => fetch(url).then(res => res.json()))
  return (
    <div>
      <Head>
        ...
        }
        {
          (!error && data) ?
            <Tile>
              ...
              {
                Object.entries(data).map(kv => <p><strong>{kv[0]}:</strong> {kv[1]}</p>)
              }
            </Tile> : 
            null
        }
