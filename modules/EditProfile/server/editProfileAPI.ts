...
export const editProfileAPI: NextApiHandler<IEditProfileResponse> = async (
  ...
  if (req.body) {
    ...
    const street = Array.isArray(query?.street)
      ? query.street[0]
      : query?.street;
    const forename = Array.isArray(query?.forename)
      ? query.forename[0]
      : query?.forename;
    const surname = Array.isArray(query?.surname)
      ? query.surname[0]
      : query?.surname;
    const city = Array.isArray(query?.city) ? query.city[0] : query?.city;
    const house_number = Array.isArray(query?.house_number)
      ? query.house_number[0]
      : query?.house_number;
    ...
    const postal_code = Array.isArray(query?.postal_code)
      ? query.postal_code[0]
      : query?.postal_code;
    const prefix = Array.isArray(query?.prefix)
      ? query.prefix[0]
      : query?.prefix;
    const suffix = Array.isArray(query?.suffix)
      ? query.suffix[0]
      : query?.suffix;
    const first_pregnancy = Array.isArray(query?.first_pregnancy)
      ? query.first_pregnancy[0]
      : query?.first_pregnancy;
    const room = Array.isArray(query?.room) ? query.room[0] : query?.room;
    const website = Array.isArray(query?.website)
      ? query.website[0]
      : query?.website;
    const consultation_hours = Array.isArray(query?.consultation_hours)
      ? query.consultation_hours[0]
      : query?.consultation_hours;
    const phone_private = Array.isArray(query?.phone_private)
      ? query.phone_private[0]
      : query?.phone_private;
