...
import { getAuthHeader, getServiceAuth, IAuthHeader } from '@/modules/auth/server';
...
export async function getServiceClient(endpoint = GQLEndpoint.Public) {
  ...
  const headers = await getServiceAuth(process.env.SERVICE_USER, process.env.SERVICE_SECRET);
  if (headers)
    return getClient(endpoint, getAuthHeader(headers));
